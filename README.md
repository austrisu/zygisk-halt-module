...
Halts module by starting infinite loop, IDA can be attached to halted process to examine during zygote hatching

After running `system_server` need to be stopped so that it doesn't kill the process which is failed to attach to `ActiityManager`

Simplest way:

```
kill -s 18 $(pidof system_server)
```

IDA should be attached to the zygote process which is stopped.
